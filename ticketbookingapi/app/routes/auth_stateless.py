"""
Authentication routes with STATELESS JWT refresh tokens (no database storage)
Alternative implementation - use this instead of auth.py if you prefer stateless approach
"""
...
def login() -> Tuple[Dict[str, Any], int]:
    """
    Đăng nhập người dùng - returns stateless JWT tokens
    """
    try:
...
def refresh_token() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        # Verify refresh token (stateless - no database query)
...
def logout() -> Tuple[Dict[str, Any], int]:
    """
    Logout user by revoking refresh token (add to blacklist)
    """
    try:
        data = request.get_json() or {}
        refresh_token_str = data.get('refresh_token')
...
def change_password() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        # Note: In stateless approach, we can't revoke all existing tokens
        # But tokens will expire naturally. For better security, consider
        # implementing a token version or last_password_change timestamp

...
import { Button, Typography, Tag, Modal, InputNumber, Input, Row, Col, Tooltip } from 'antd';
import { SaveOutlined, PlusOutlined, DeleteOutlined, AppstoreOutlined, StopOutlined } from '@ant-design/icons';
...
const VenueSeatMapEditor = ({ venueName, initialAreas, visible, onClose, onSave, saving }) => {
    ...
    const handleSeatMouseDown = (areaIndex, row, col) => {
        ...
        const action = area.locked_seats.includes(seatId) ? 'unlock' : 'lock';
    ...
    const renderSeatMap = (area, areaIndex) => {
        const rows = [];
        for (let r = 1; r <= area.rows; r += 1) {
            const cols = [];
            for (let c = 1; c <= area.cols; c += 1) {
                ...
                >
                    ...
                    <div style={{ marginTop: 16 }}>
                        {areas.map((area, idx) => (
                            ...
                            >
                                <Text strong style={{ color: activeAreaIndex === idx ? '#2DC275' : 'inherit' }}>
                                    {area.name}
                                </Text>
                                <br />
                                <Text type="secondary" style={{ fontSize: 11 }}>
                                    {area.rows} x {area.cols} ({area.rows * area.cols} ghế)
                                </Text>
                                <Button
                                    type="text"
                                    size="small"
